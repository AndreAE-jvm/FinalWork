/* Пример 1.4. Для задания, рассмотренного в предыдущем примере, разработать методы
для выполнения каждой подзадачи:
1) ввод массива стран – метод setCountry(...);
2) вывод информации о странах в табличном виде – метод showArray(...);
3) вывод информации об одной стране – метод showCountry(…);
4) определение индекса страны с максимальной площадью – метод nomMax(…);
5) сортировка стран по возрастанию площади – sortSquare(…);
6) сортировка стран по названию в алфавитном порядке – метод
sortName(…);
7) определение средней площади – метод avgSquare(…);
8) определение страны с площадью больше средней – метод bigger(…);
9) поиск страны по названию – метод findForName(…). */
